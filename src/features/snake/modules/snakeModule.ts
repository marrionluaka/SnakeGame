...
const _willEat = (state: GameState): boolean => equals(_getNextHead(state), state.bait)
...
const _getNextBait = (state: GameState): Point => (_willEat(state) ? _getNextBaitPosition(state) : state.bait)
...
const _getNextBaitPosition = (grid: Grid): Point => ({ x: _random(0, grid.cols - 1), y: _random(0, grid.rows - 1) })

...
describe('snakeModule specs', () => {
  ...
  it("returns the bait's current location", () => {
    const nextState = getNextState({ ...initialState, bait: { x: 12, y: 4 } } as unknown as GameState)
    expect(nextState).toEqual(nextState)
  })

  it('returns a new bait at a random location', () => {
    ...
    expect(nextState.bait).toEqual({ x: 5, y: 3 })
  })
  ...
  it('returns the next state of the snake when it is about to eat (expands the snake)', () => {
    ...
    const nextState = getNextState({ ...initialState, snake: [{ x: 14, y: 7 }], bait: snakeHead } as unknown as GameState)
